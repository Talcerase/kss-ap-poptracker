...
kirby_super_star_location = {}
kirby_super_star_location.__index = kirby_super_star_location
...
function kirby_super_star_location.new(name)
    local self = setmetatable({}, kirby_super_star_location)
    if name then
...
function kirby_super_star_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = kirby_super_star_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function kirby_super_star_location:connect_two_ways(exit, rule)
...
function kirby_super_star_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function kirby_super_star_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function kirby_super_star_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function kirby_super_star_location:accessibility()
...
-- 
function kirby_super_star_location:discover(accessibility, keys)
...
entry_point = kirby_super_star_location.new("entry_point")
